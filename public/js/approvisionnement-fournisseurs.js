 ...
 *
 * Design: "Ledger wall → Data book"
 *   Landing: supplier list styled as ledger spines on a dispatch rail.
 *   Detail:  tabbed "data book" with KPI cards, sparklines, inline invoice
 *            preview panel, and MI catalogue.
 */
'use strict';
...
let activeTab       = 'overview';   /* 'overview' | 'factures' | 'catalogue' */
let inlineOpenId    = null;         /* invoice id whose inline preview is expanded */
...
/* GL category → accent color for ledger spine tab */
const GL_ACCENT = {
  '4101': '#c8a060', /* Malt */
  '4102': '#9eb060', /* Hops */
  '4103': '#6593b8', /* Yeast */
  '4104': '#b0905a', /* Adjuncts */
  '4201': '#9b7cc8', /* Packaging */
  '4202': '#9b7cc8',
  '4401': '#60aab0', /* Cleaning */
  '4500': '#9880c8', /* R&D */
  '4510': '#9880c8',
  '4701': '#8a9870', /* Utilities */
  '6100': '#b8bcc0', /* Maintenance */
  '6281': '#7888a0', /* Transport */
  '1302': '#a07a48', /* Cautions */
};
function glAccent(gl) {
  return GL_ACCENT[String(gl)] || '#7a6647';
}

/* Volume: from stats.total_chf */
...
function filteredSuppliers() {
  ...
  else if (currentFilter === 'parser')    result = result.filter(s => s.parser_key);
...
function renderList() {
  ...
  /* Find max volume for the volume bar scaling */
  ...
  list.innerHTML = items.map(s => {
    ...
    const parserMark = s.parser_key
      ? `<span class="af-ldg-chip af-ldg-chip-parser" title="Parser: ${escHtml(s.parser_key)}">P</span>`
      : '';
...
/* Build a small SVG polyline from an array of {date, value} sorted asc */
function buildSparkline(points, w, h, color) {
  ...
  return `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" aria-hidden="true" preserveAspectRatio="none">
    <polyline points="${pts}" fill="none" stroke="${color}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" opacity=".8"/>
  </svg>`;
}

/* Derive monthly invoice totals from the invoices array for a sparkline */
function invoiceSparkPoints(invoices) {
  /* Group by YYYY-MM, sum total_ht (or total_ttc as fallback) */
  const monthly = {};
...
function glDonut(fp, r) {
  /* fp: [{gl, chf}, …]; r = outer radius; returns SVG string */
  const size = r * 2 + 4;
  const cx   = r + 2;
  const cy   = r + 2;
  const ir   = r * 0.55;
  ...
  if (!total) return '';
  const colors = [
    'var(--dock)','var(--hop)','var(--bbt)','var(--ember)','var(--oak)','var(--steel-mid)',
  ];
  let startAngle = -Math.PI / 2;
  const slices = fp.map((item, i) => {
    ...
    const col = glAccent(item.gl) !== '#7a6647' ? glAccent(item.gl) : colors[i % colors.length];
...
function renderGlFootprint(s) {
  ...
  const barsHtml = fp.map((item, i) => {
    ...
    const col     = glAccent(item.gl) !== '#7a6647' ? glAccent(item.gl) : barColors[i % barColors.length];
...
function renderOverviewPane(s) {
  ...
  /* Last delivery approx: max date from invoices as proxy */
  const lastDelivDate = lastInvDate;

  /* Sparkline from invoice history */
  const sparkPts = invoiceSparkPoints(invoices);
  const sparkSvg = buildSparkline(sparkPts, 80, 28, 'var(--dock)');

  /* Donut for GL footprint */
  ...
  const parserHtml = s.parser_key
    ...
    : `<div class="af-doc-no-parser">Pas de parseur</div>`;
  ...
  return `
    ...
    <div class="af-kpi-row">
      ...
      <div class="af-kpi-card">
        ...
        <div class="af-kpi-sub">date invoice la plus récente</div>
      </div>
    </div>
    ...
    <div class="af-ov-fields">
      ...
      <div class="af-ov-field">
        ...
        <div class="af-ov-field-val mono">${s.vat_number ? escHtml(s.vat_number) : '<span style="color:var(--ink-faint);font-style:italic">—</span>'}</div>
      </div>
      <div class="af-ov-field">
    ...
    <!-- Invoice activity timeline (mini bar chart) -->
    ${sparkPts.length > 1 ? renderActivityChart(sparkPts) : ''}
  `;
}

/* ── Activity bar chart (monthly invoice spend) ──────────────────── */
...
function renderActivityChart(pts) {
  ...
  const bars = pts.map((p, i) => {
    ...
    const x   = i * (barW + 2);
    const y   = H - h;
    const col = p.value === maxV ? 'var(--dock)' : 'rgba(200,134,58,.45)';
...
function renderFacturesPane(s) {
  const invoices = s.invoices || [];
  if (!invoices.length) {
    return `<div class="af-empty-catalogue" style="padding:32px 20px;font-size:13px;">
      Aucune facture enregistrée pour ce fournisseur.</div>`;
  }
  ...
  const rows = invoices.map(inv => {
    ...
    const previewBtn = hasPdf
      ? `<button class="af-inv-row-preview-btn${isOpen ? ' active' : ''}" data-inv-id="${inv.id}" data-file-id="${escHtml(inv.drive_file_id)}" data-ref="${escHtml(ref)}" aria-label="Aperçu ${escHtml(ref)}" title="Aperçu inline">
    ...
    const fullscreenBtn = hasPdf
      ? `<button class="af-inv-row-fs-btn" data-file-id="${escHtml(inv.drive_file_id)}" data-ref="${escHtml(ref)}" aria-label="Plein écran" title="Ouvrir en plein écran">
    ...
    /* Inline preview panel — only rendered when open, image NOT lazy */
            ...
            <div class="af-inv-inline-panel">
              <div class="af-inv-inline-header">
                ...
                <div class="af-inv-inline-actions">
                  <a class="af-inv-pdf-btn af-inv-pdf-btn-sm" href="/api/document.php?file_id=${encodeURIComponent(inv.drive_file_id)}" target="_blank" rel="noopener">PDF complet</a>
...
/* ── Catalogue MI tab content ────────────────────────────────────── */

function renderCataloguePane(s) {
  const catalogue = s.catalogue || [];
  if (!catalogue.length) {
    return `<div class="af-empty-catalogue" style="padding:32px 20px;font-size:13px;">
      Aucun MI observé — aucune livraison Active/Consumed enregistrée.</div>`;
  }
  return `
...
function openFiche(id) {
  ...
  <div class="af-databook" style="--db-accent:${accent}">
    ...
    <!-- Data book cover band -->
    <div class="af-db-cover">
      <div class="af-db-cover-spine"></div>
      <div class="af-db-cover-body">
        <div class="af-db-cover-stamp">carnet fournisseur · lecture seule</div>
    ...
    <div class="af-db-tabs" role="tablist">
      <button class="af-db-tab${activeTab === 'overview' ? ' active' : ''}" data-tab="overview" role="tab" aria-selected="${activeTab === 'overview'}">
      ...
      <button class="af-db-tab${activeTab === 'factures' ? ' active' : ''}" data-tab="factures" role="tab" aria-selected="${activeTab === 'factures'}">
        <svg width="12" height="12" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      ...
      <button class="af-db-tab${activeTab === 'catalogue' ? ' active' : ''}" data-tab="catalogue" role="tab" aria-selected="${activeTab === 'catalogue'}">
        <svg width="12" height="12" viewBox="0 0 14 14" fill="none" aria-hidden="true">
  ...
  /* ── Wire inline preview buttons (factures pane) ── */
...
function toggleInlinePreview(invId, fileId, ref, s) {
  if (inlineOpenId === invId) {
    /* Close: re-render factures pane without expansion */
    inlineOpenId = null;
  } else {
    inlineOpenId = invId;
  }
  /* Re-render factures pane in place */
  const pane = document.querySelector('.af-db-pane[data-pane="factures"]');
  if (!pane) return;
  pane.innerHTML = renderFacturesPane(s);
  /* Wire buttons again after re-render */
  const fiche = document.getElementById('af-fiche');
  if (fiche) wireFactureButtons(fiche, s);

  /* Load the inline preview image if now open */
  if (inlineOpenId === invId) {
    ...
    if (!img) return;
    /* NOT lazy — image starts hidden but we set src immediately */
    img.onload = () => {
...
/* Zoom state — reset on every openInvoiceViewer call */
const _afZoom = {
  scale: 1,
  panX: 0,
  panY: 0,
...
function openInvoiceViewer(fileId, invoiceRef) {
  ...
  modal.innerHTML = `
    <div class="af-inv-dialog">
      <div class="af-inv-titlebar">
        <span class="af-inv-titlebar-ref">${safeRef}</span>
      ...
      <div class="af-inv-body">
        <div class="af-inv-img-wrap">
          <div class="af-inv-loading" id="af-inv-loading">
            <span class="af-inv-spinner"></span>
            <span>Chargement...</span>
          </div>
          <img
            class="af-inv-preview"
            id="af-inv-preview"
            alt="Apercu document"
            style="display:none"
          >
          <div class="af-inv-err" id="af-inv-err" style="display:none">Apercu indisponible</div>
          <div class="af-zoom-bar" id="af-zoom-bar" style="display:none" aria-label="Contrôles zoom">
            ...
            <button class="af-zoom-btn af-zoom-fit" id="af-zoom-fit" aria-label="Ajuster à la fenêtre" title="Ajuster">
              <svg width="11" height="11" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                <polyline points="1,4 1,1 4,1" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
                <polyline points="8,1 11,1 11,4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
                <polyline points="11,8 11,11 8,11" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
  ...
  function applyZoom(animated) {
    ...
    if (_afZoom.scale <= _afZoom.minScale) wrap.style.cursor = 'default';
  }
  ...
  function maybeUpgradeHiRes() {
    if (_afZoom.scale <= 1.5 || hiResRequested) return;
    hiResRequested = true;
  ...
  btnFit.addEventListener('click', () => {
    _afZoom.scale = _afZoom.minScale;
    _afZoom.panX  = 0;
    _afZoom.panY  = 0;
    applyZoom(true);
  });
