...
class SenderParser(abc.ABC):
    ...
    Subclasses MUST implement:
      ...
      matches (EmailContext → bool)
      parse   (EmailContext → ParsedOrder)

    Dispatch convention (mirrors lib/invoice-parsers/index.js):
      registry.dispatch() iterates the registered parsers in declaration order.
      The FIRST parser whose matches() returns True wins; its parse() is called.
      If parse() raises, the caller catches and marks parse_status='error'.
      If no parser matches, the caller marks parse_status='no_match'.
    ...
    Parsing discipline:
      ...
      • Raise ValueError (or any exception) on parse failure; the caller surfaces
        the message as parse_status='error'.  Don't swallow exceptions silently.
