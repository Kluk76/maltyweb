---
name: sql
description: Write, refactor, or debug SQL — MySQL DDL/DML/queries — for the canonical maltytask `maltytask` database, maltyweb migrations under `db/migrations/`, and PHP-backed read paths. Equip whenever a task involves SELECT/INSERT/UPDATE/DELETE on the production DB, schema migrations, query work inside PHP modules (`/modules/*.php`), report SQL, backfill scripts, or ad-hoc verification queries. The body encodes the MySQL-8 bugs we've actually hit (ONLY_FULL_GROUP_BY, collation mismatch utf8mb4_0900_ai_ci vs _unicode_ci, FK type mismatch BIGINT-vs-INT-UNSIGNED, ENUM truncation, PDO :named-vs-?, INSERT IGNORE swallowing upserts, DECIMAL drift, NULL fall-through, VARCHAR-vs-INT-literal coercion, the broken row-index bridge, soft-delete-flag-not-enforced, event-sourced-replay stale-binding drain, corrective-upsert NK-preservation — 26 in all), the maltytask write discipline (`withConnection`/`logAudit`/snapshot/`--dry-run`-default, EUR→CHF 0.945, row_hash idempotency, `ANY_VALUE` for functional dependency, period='YYYY-MM', `ref_mi.is_inventoried` filter), verification recipes (`php -l` via VPS, dry-run, count-then-apply, re-run idempotency), and performance/operations craft (EXPLAIN ANALYZE, composite/covering indexing, deadlock retry 1213/1205 around FOR UPDATE, `conn.execute` vs `query`, INSTANT/INPLACE DDL, generated columns, ENFORCED CHECK + FK ON DELETE, observability, least-privilege grants, bulk-load, backup/PITR). Layers with `coder` or `parser-coder` — equip in addition whenever SQL is in scope.
---
...
|---|---|
| Debugging a query/migration that misbehaves, or final review | `references/anti-patterns.md` — 29 real bugs, symptom→fix (indexed) |
