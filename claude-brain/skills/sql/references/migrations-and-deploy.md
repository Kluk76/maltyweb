...
10. **No result-set-leaving statement in a `migrate.php` file — it poisons the NEXT statement with error 2014.** `migrate.php` runs each `.sql` through a single PDO `exec()` and then INSERTs into `schema_migrations` on the same connection. Any statement that produces a result set — a bare `SELECT`, **`CREATE OR REPLACE VIEW`**, `SHOW`, `EXPLAIN`, `CALL` of a procedure that selects — leaves an *unfetched* result pending on the connection. The very next statement (the migration's own next line, or migrate.php's `schema_migrations` INSERT) then fails with `SQLSTATE[HY000] 2014: Cannot execute queries while other unbuffered queries are active`. This is the same single-`exec()` root cause as the multi-statement-DDL gotcha (#9), but the trigger is a result set rather than a mid-file abort, so it bites even when every statement is individually valid. Confirmed live on migs 250/251 (`CREATE OR REPLACE VIEW v_bd_packaging_v2_vendable`).
    - **Mitigation when a migration must touch a view:** apply it OUT-OF-BAND, statement-by-statement, via `php -r` (or `mysql` client) so each statement runs on its own clean cursor, then INSERT the `schema_migrations` row manually. The `.sql` file stays the source-of-record, but note in the commit body that it will NOT re-apply cleanly through `migrate.php` on a fresh environment.
    - **Hardening candidate (not yet done):** have `migrate.php` call `$stmt->closeCursor()` (or split on `;` and run via `query()` + drain) between statements so result-set-leaving statements no longer poison the next one. Until that lands, treat any `CREATE OR REPLACE VIEW` / `SELECT` / `SHOW` inside a migration file as the out-of-band path above.
