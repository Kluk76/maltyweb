...
> **✅ FULL BUILD SHIPPED + LIVE 2026-06-19 — all 5 waves, smoke 8/8 PASS.** Migs 409 (6 tables + ref_suppliers.criticality ENUM + EF-01 grid seed A22/B13, thresholds 75/50) + 410 (KO flags A2/A5). Server-side scoring engine `app/supplier-eval-helpers.php`; Évaluation section in `#sf-fiche`; cron `send-supplier-review-reminders.php` (deployed DISABLED) + CSV export. Criticality seeded 24 critique / 35 non_critique / 77 NULL. Commits `d0a11dc`/`561ba4f`/`7b10538`/`b522ec8` LOCAL-only (push deferred to Kouros). NON-FISCAL observation layer. Carry-forward: operator enables cron + sets system_settings; first real evaluation campaign = operator/quality-team. → §BUILD STATUS for full as-built + carry-forward.
...
- **✅ W4 (periodic-review cron + auditor CSV export) = SHIPPED** (see FULL-BUILD block) — `scripts/send-supplier-review-reminders.php` (deployed DISABLED, --dry-run default) + `public/api/supplier-evaluation-export.php` (pure-PHP CSV, 11 cols, critique-first). Commit `b522ec8`.
- **✅ W5 (smoke + verify) = DONE, 8/8 PASS** (see FULL-BUILD block) — render/happy-path(68.57%==displayed, Opus hand-calc)/KO/supersession/criticality-override/NC/cert/CSV; all test rows self-cleaned, distribution restored 24/35/77.
