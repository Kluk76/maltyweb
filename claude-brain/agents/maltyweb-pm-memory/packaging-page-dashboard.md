...
## ✅ PARALLEL-RUN BLANK-VOLUME GUARD on the WEB packaging form — SHIPPED + DEPLOYED 2026-06-16 (NOT committed; tree left dirty 3-file, Kouros commits)
> **AS-BUILT 2026-06-16** — built per the brief below, BOTH layers, deployed via `bin/deploy.sh --apply`, md5-verified local↔VPS byte-identical, `php -l` clean on both PHP files. Three files:
> 1. **`public/modules/form-packaging.php`** — server-side guard in the per-format loop after the qty coercion (~L957). Fires ONLY when `!$isReassignerMode && $fReusesPackagingIdFk === null` (réassigner + cuve-réutilisée carve-outs honored). Main row (`$fOrigin==='main'`): throws `RuntimeException "Run principal : volume (unités) manquant ou nul."` if `$fProdTotal === null || <= 0`. Parallel: `"Format parallèle #{$idx} : volume (unités) manquant ou nul."` if `$fQteUnites === null || <= 0`. Caught → `flash_set('err')` + PRG redirect, nothing written (NK preserved). **This is the load-bearing fix — it re-fires on EVERY corriger-loop partial save** (the standing-form-fact discipline).
> 2. **`public/js/packaging-form.js`** — new form-submit listener (BEFORE the CO2/O2 guard) HARD-blocks (`preventDefault`) with a named inline error, scrolls+focuses the offending input. Same carve-outs (skips réassigner + `rowIsReuse()` rows).
> 3. **`public/css/packaging-form.css`** — `.pf-qty-error` style (ember, DM Sans), mirrors racking form's inline-error pattern.
>
> **🔴 ONE HONEST GAP (first live validation = Xavier's next packaging submission):** the four behavioural smoke cases (a blank→reject, b zero→reject, c valid→pass, d réassigner/reuse→not blocked) were NOT run live — the packaging form is operator-floor hard-gated (smoketest viewer → 403) and a real submit would write live `bd_packaging_v2` rows. **Verified at CODE level only:** PHP mirrors the existing `$mainCount !== 1` throw pattern exactly; JS reads the same field names with identical carve-outs. (Acceptable — this is the same operator-floor smoke constraint that recurs for write-path forms; the corriger-loop re-fire makes the server guard the real backstop regardless.)
>
> **GIT:** NOT committed — concurrent sessions share the clone; tree left dirty with ONLY these 3 intended edits; Kouros handles the commit (commit-by-pathspec discipline applies).
>
> ### Original PM brief (kept for the design rationale)
> Incident: Xavier submitted a ZEPV (cuv) session WITH a parallel run, forgot the parallel leg's volume → silently inserted a 0-volume parallel row (vendable_hl null/0); Kouros fixed by hand. Operator wants submission FORBIDDEN when ANY parallel run lacks its volume. Form = `public/modules/form-packaging.php` (renderer + POST handler) + `public/js/packaging-form.js` (DOM). **NO direct HL field — "volume" = the per-row UNIT COUNT** (`formats[N][prod_total_units]` on MAIN, `formats[N][qte_unites]` on PARALLEL); HL is DERIVED from units via SKU_BOM in `compute_packaging_vendable_hl()`. Blank-vs-0 distinction is at form-packaging.php L950-953 (`isset && !== '' ? (int) : null`). Parallel qty → prod_total_units at L1091/L1163/L1240. Gap: client-side `tryEnableSubmit()` (L854-873) only checks tank+≥1 format, NEVER per-row qty; server coerces blank→null and inserts. **Verdict: BOTH layers.** Client = mirror the CO2-inversion guard (`form.addEventListener('submit',…)` L1499+, but HARD-block here — this is a required field, not import-then-flag) — for every `.pf-format-row` with `row_origin='parallel'`, require its qty input (`[name$="[qte_unites]"]`) non-blank AND >0; on fail `e.preventDefault()` + inline `.op-form__error` + focus. Server (defense-in-depth, the corriger-loop re-fires it) = inside the format loop after L953, when `$fOrigin==='parallel'` and `$fQteUnites` is null-or-≤0 → `throw new RuntimeException("Format parallèle #{$idx} : volume (unités) manquant.")` (mirrors the existing main-count / suffix throws → flash_set+redirect, NK preserved). Refuse-don't-NULL: never insert a 0-unit parallel row. Apply SAME guard to MAIN's `prod_total_units` (a blank main qty is the same silent-zero bug). **Distinguish blank from legit 0:** a packaging run with 0 units is meaningless → treat both blank AND 0 as invalid for prod qty (unlike objective_hl which is optional). DO NOT gate réassigner-mode legs (no new volume) or cuve-réutilisée rows (`reuses_packaging_id_fk` set → no volume) — those legitimately carry no qty. EQUIP ui+coder (webapp-testing smoke via corriger-loop). NB the legacy RawDB Mode-A defect (packaging-parallel-run-normalizer-defect.md) is a DIFFERENT, already-shipped fix (read-side normalizer) — this is the WEB write-side; don't conflate.
