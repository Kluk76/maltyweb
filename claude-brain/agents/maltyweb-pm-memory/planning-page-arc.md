# Planning Page Arc — schema, engine, conventions (load when touching planning) — 🔴 ONE ACTIVE REWORK PENDING (serving-tank occupancy); rounds 1-8 ALL SHIPPED+LIVE+PUSHED

---

# 🔴🔴 NEXT / ACTIVE REWORK — SERVING-TANK (CUVE DE SERVICE) OCCUPANCY MODEL — read this FIRST (cold-resume-ready, self-contained)

> **STATUS: Round-8 occupancy estimate is SHIPPED + LIVE, but operator (Kouros) has explicitly flagged it for REWORK — _"il va falloir qu'on retravaille l'occupation des cuves … on s'y attèle plus tard."_ Captured 2026-06-19 before a chat-clear. This is the one open piece of NET-NEW design on the planner arc; everything else (rounds 1-8) is shipped + pushed. Likely its own sub-arc when picked up.**

### WHAT is shipped now (the thing to rework)
**Round-8 Piece B** — `predict_load_free_serving_tank_count($pdo, DateTimeImmutable $asOf)` in **`app/planning-predict.php`** computes:
- `free = max(0, in_house_active − occupied)`, where `in_house_active` = `ref_serving_tanks WHERE location='in_house' AND status='active'` (= **8** today).
- `occupied` = COUNT of `bd_packaging_v2` rows `run_type='cuv'` (NOT repack, NOT tombstoned) with `event_date ∈ (asOf−N, asOf]`.
- **N = `system_settings serving_tank_turnaround_days`** (section `production_targets`, default **14**, tunable), seeded by **migration 414** (`414_serving_tank_turnaround_setting.sql`, APPLIED on VPS).
- Consumed by the producer ONLY as a **secondary weekly cap** on serving-tank proposals (the Round-5 client-recurrence trigger is the primary). Commit `c104e28` (maltyweb main, PUSHED). It is an explicit **ESTIMATE** (count-based N-day decay, code-commented as such).

### WHY it needs rework (consolidated known defects — all true, do not re-discover)
1. **N is "très aléatoire" per operator** — there is no canonical cuve turnaround. The default **N=14 currently yields occupied=9 > 8-tank fleet → free=0 this week**, so the planner DEFERS all cuve proposals → it **OVER-SUPPRESSES**. (Sensitivity at asOf 2026-06-16: N=14→free=0; N=7→free=4; N=21→free=0.) Kouros must calibrate N — but calibration is a band-aid, not the fix.
2. **The decay heuristic DOUBLE-COUNTS:** a tank re-filled inside the N-day window counts as 2 occupied. That is exactly why 9 fills in 14 days exceed the 8-tank fleet.
3. **ROOT CAUSE — there is NO cuve return/pickup event in the data.** The proper fix is to **CAPTURE one** (a "retour cuve" logistics form → an event table) and then model TRUE occupancy (filled → at-client → returned) instead of a time-decay guess. VERIFIED 2026-06-19: cuve FILLED = `bd_packaging_v2 run_type='cuv'` exists, but EMPTY/at-client/return/pickup = **NO event, NO table** anywhere (`ord_returns`/`v_physical_returns` = the FG **sales**-return arc, unrelated). Also **NO physical-tank-id column** on any cuv event (the only `yt_number` cols are YEAST tanks) → per-physical-tank occupancy is NOT derivable from current data. This is the real rework and likely its own arc (logistics-capture + true occupancy model).
4. **ARCHITECTURE: occupancy currently lives PRODUCER-SIDE** (`predict_load_free_serving_tank_count` in `app/planning-predict.php`), **NOT in `TankSimulator::run()`.** The Round-8 ruling NAMED a new `serving` block in `TankSimulator::run()` (symmetric with cct/bbt); the build deviated to producer-side (PM-ratified — single consumer, minimise blast radius under parallel sessions). **PROMOTE-TRIGGER: if the rework needs eligibility (or any 2nd consumer) to also see serving occupancy, MOVE the derivation into a `TankSimulator::run()` `serving` block** — at that point the producer-local reader becomes a divergent reader and must be hoisted to the canonical sim. (Leave a redirect comment at the dead `~L648` TODO — it's in the racking dest-switch `dest_type==='YT'` which has 0 rows; cuves are filled via the PACKAGING path, not racking, so do NOT build serving occupancy at L648.)
5. **LATENT DIVERGENCE to resolve in the rework — THREE client ID spaces, no join:** `bd_packaging_v2.client_fk → ref_packaging_clients` (physical fill: MICA/Nausikraft/… id 1/2/18/20) vs `ref_customers.is_serving_tank_client` (the recurrence roster: 845/2612/1827/1848/6) vs legacy `ref_clients`. The cuv-fill event and the serving-tank-client roster **DO NOT JOIN today.** Doesn't block the count (count needs no client join), but a true occupancy model that ties a fill to a client WILL need this reconciled. → derivation-tree-and-schema.md if it surfaces.

### REWORK SHAPE (recommendation when picked up — not yet ruled)
- **Proper fix = capture a real cuve return/pickup event** ("retour cuve" form → an event table keyed on the physical tank + client + date), then derive occupancy as filled-minus-returned instead of N-day decay. That kills both defect #2 (double-count) and the N-guess (defect #1).
- **Interim** (if needed before the form lands): de-duplicate the decay count by physical tank, or have Kouros calibrate N — but flag both as band-aids on a heuristic with no real empty signal.
- Reconcile the **three client ID spaces** (defect #5) as part of tying fills to clients.
- If a 2nd consumer needs occupancy → hoist into `TankSimulator::run()` `serving` block (defect #4 promote-trigger).
- EQUIP **coder + sql** (+ webapp-testing only within the still-open master manager UAT). CARDINAL holds: planning is INTENT — writes nothing to COGS/COP/WAC/BOM/beer-tax/inventory. Re-`migrate.php --status` at build-start (head ≥414; the number leads).

**Full Round-8 detail (fill/empty event mapping, sensitivity, ratified deviation) → §ROUND-8 AS-BUILT + §SERVING-TANK OCCUPANCY IN TankSimulator (Round-8 ruling) below.**

---

## 🧭 COLD-RESUME ANCHOR — whole planner arc state in one place (the rework above is the ONLY net-new design open)
- **AS-BUILT (rounds 1-8), all SHIPPED + LIVE + COMMITTED + PUSHED on maltyweb main.** Surfaces: page `public/modules/planning.php` · pure-read engine `app/planning-eligibility.php` · producer `app/planning-predict.php` · objectifs settings on `public/modules/salle-de-controle.php?sec=objectifs` · serving-tank client fiche on `public/modules/expeditions.php?view=clients`.
- **CARDINAL (enforced in schema_meta + code):** Planning is INTENT — it reads canonical state and writes ONLY to `pl_plan_days`/`pl_plan_items`; it NEVER feeds COGS/COP/WAC/BOM/beer-tax/inventory.
- **Migrations applied:** 364 (planning tables) · 365 (suggest settings) · 367 (preset access grants) · 401 (working-days + brew/packaging caps) · 407 (`ref_customers.is_serving_tank_client` + `serving_tank_cadence_days`) · 408 (`pl_plan_items.customer_id_fk`) · 411 (serving-tank fiche cols, budget = PER MONTH) · 414 (`serving_tank_turnaround_days`). MIG HEAD ≥414 — re-`migrate.php --status` at build-start, the number leads.
- **Round map:** P1 week calendar / P2 process-eligibility / P3 predictive suggestions (2026-06-15) → multi-process-per-day UX P0/P1 (`f0361d4`) → r1-3 fuller planner (tank space + packaging capacity + racks/dry-hops + serving count, `521ccbf`) → r4 core-only filter + Transferts label + bottling/canning mutex (`6eb25a7`) → r5 serving-tank client-recurrence trigger (`2e02d1d`/`652bbae`/`982f298`) → r6 serving-tank client fiche (`43abfce`/`bce4e6b`) → r7 budget→planner pace-check (`50e4eb3`) → r8 manual client picker + occupancy estimate (`4dc27c0`/`c104e28`). Full pushed chain since r3: `521ccbf→6eb25a7→2e02d1d→652bbae→982f298→43abfce→bce4e6b→50e4eb3→4dc27c0→c104e28`.
- **🔴 STILL-OPEN MASTER GATE (carried since P0/P1, now spans all 8 rounds): authenticated MANAGER browser UAT** of the whole planner — Suggérer-un-plan output incl. serving-tank cards, the manual client picker (Piece A), the salle-de-controle objectifs settings/toggles, the Round-6 serving-tank fiche block, and the Round-7 budget pace-check (once budgets are entered). Opus verifies structurally + via rollback harness only — a subagent cannot auth/click as a manager. Needs Kouros (or a manager session) to click through. EQUIP webapp-testing once a manager session exists.
- **Deferred items (by design, not defects):** (a) **editable UI for `serving_tank_turnaround_days`** — deferred to avoid a salle-de-controle parallel-edit collision; tunable via SQL for now. (b) **`serving_tank_cadence_days` override col** — UNUSED by Kouros's explicit choice (cadence stays derived-only median); NOT a defect. (c) the manual serving_tank add-form now HAS the client picker (closed in r8). (d) 3 excluded users (Louis Maechler, Thierry Stierli, Olivier Barral) pending an onboarding decision on Planning access. (e) optional predictive cron, disabled-default.
- **v1 known limitations (flagged to Kouros):** serving-tank free-count is the N-day decay ESTIMATE above (the active rework); dry-hop coverage PARTIAL (9 spec-flagged recipes vs ~12+ observed — under-proposes by design, refuse-don't-guess); packaging throughput = HL/week budget only (`speed_units_h` débit path deferred).
...
## §ROUND-8 — ✅ SHIPPED + DEPLOYED + COMMITTED + PUSHED 2026-06-19 (maltyweb main). TWO pieces (Kouros chose both). Closes open-followup #2 (manual client picker) + parameterises (does NOT eliminate) the v1 empty-at-week-start free-serving-tank limitation. Cadence-override wiring EXPLICITLY DECLINED by Kouros — `serving_tank_cadence_days` STAYS UNUSED (not a defect).

**PIECE A — Manual serving-tank client picker (closes open follow-up #2).** Files: `public/modules/planning.php` + `public/js/planning.js`. The manual `add_packaging` form now shows a cuve-de-service client `<select name=customer_id_fk>` (populated from `ref_customers WHERE is_serving_tank_client=1 AND is_active=1`) **ONLY when `pkg_type=serving_tank`** (JS toggle mirroring the wort progressive-disclosure idiom). Handler validates the id is a real serving-tank client and persists `customer_id_fk` (NULL for any non-serving_tank row). Render already displayed it (Round-5). Commit `4dc27c0`, pushed. Verified (rollback insert customer_id_fk=6). → manual serving_tank rows are no longer customer-NULL.

**PIECE B — Occupancy-aware free serving-tank count (parameterises, does NOT eliminate, the empty-at-week-start limitation).**
- ⚠️ **ARCHITECTURE DEVIATION from the Round-8 ruling below (PM ratified after-the-fact).** The ruling said "new `serving` block in `TankSimulator::run()`". The build agent implemented it **PRODUCER-SIDE** instead: `predict_load_free_serving_tank_count($pdo, DateTimeImmutable $asOf)` in `app/planning-predict.php` now returns `max(0, in_house_active − occupied)`, where `occupied = COUNT bd_packaging_v2 run_type='cuv'` (NOT repack, NOT tombstoned) with `event_date in (asOf−N, asOf]`. **PM verdict: ACCEPTED — the SUBSTANCE of the ruling is met (count-based decay, pure-read, no per-tank identity); only the LOCATION differs.** Agent rationale (sound): the occupancy is consumed by ONLY the producer's secondary cap → threading it through TankSimulator + the eligibility engine would touch 3 files for no other consumer, and parallel sessions on salle-de-controle/sim made minimising blast radius prudent. 🔴 **FUTURE-ME promote-trigger: if eligibility (or any second consumer) ever needs serving occupancy, MOVE this into the `TankSimulator::run()` `serving` block then** — at that point the producer-local derivation becomes a divergent reader and must be hoisted to the canonical sim. Until then, producer-local is the right altitude.
- **N = `system_settings serving_tank_turnaround_days`** (section `production_targets`, default **14**, tunable), seeded by **mig `414_serving_tank_turnaround_setting.sql` (APPLIED).** 🔴 NB the ruling named the setting `serving_tank_tenure_days` — the build used `serving_tank_turnaround_days`; the latter is the live key. Kouros: real turnaround is "très aléatoire, dur à dire" → it's an explicit ESTIMATE (code-commented as such). Mig number = **414** (parallel sessions led past 411/412/413 — the number leads, re-`--status` always).
- **NO editable UI for the setting this round** — avoided `salle-de-controle.php` (parallel session editing it). Tunable via SQL only for now; editable UI = deferred follow-up.
- Commit `c104e28`, pushed.
- 🔴 **CALIBRATION FLAG to Kouros — default N=14 currently ZEROES free cuves.** SENSITIVITY at asOf 2026-06-16, 8 in-house tanks: N=14 → occupied=9 → **free=0** (fleet shows fully in circulation, planner DEFERS all cuve proposals this week); N=7 → free=4; N=21 → free=0. The default 14 may OVER-SUPPRESS — Kouros must calibrate N against real turnaround. (Reflects the count-based decay heuristic's known imprecision: 9 fills in 14 days > 8-tank fleet because the decay window double-counts tanks re-filled inside N days. Real fix = a cuve return/pickup event — deferred, see below.)

**Full pushed commit chain since round-3 (ALL on maltyweb main, PUSHED):** `521ccbf`→`6eb25a7`→`2e02d1d`→`652bbae`→`982f298`→`43abfce`→`bce4e6b`→`50e4eb3`→`4dc27c0`→`c104e28`.

🔴 **OPEN FOLLOW-UPS after Round-8 (carry on arc):**
- **Editable UI for `serving_tank_turnaround_days`** — deferred (salle-de-controle.php parallel-edit collision). Tunable via SQL for now.
- **Default N=14 may over-constrain** (0 free this week) — needs operator calibration. The PROPER fix = a real **cuve return/pickup event** (bigger logistics capture — a "retour cuve" form), which would replace the decay heuristic with actual empty signals; deferred.
- 🔴 **DIVERGENCE BACKLOG (PM flag, carried from the Round-8 ruling):** `bd_packaging_v2.client_fk → ref_packaging_clients` vs the serving-tank flag on `ref_customers` vs legacy `ref_clients` = **THREE client ID spaces, no join.** Does NOT block (the count needs no client join) but is a latent two-stores-one-fact. → derivation-tree-and-schema.md if it surfaces.
- **`serving_tank_cadence_days` override col UNUSED** — by Kouros's explicit choice this round, NOT a defect. (Cadence stays derived-only.)
- **v1 empty-at-week-start free-count is now PARAMETERISED (N-day decay), not eliminated** — still an estimate, not a real return signal.
- 🔴 **MASTER GATE STILL OPEN (now spans 8 rounds) — authenticated MANAGER browser UAT of the WHOLE arc:** planner all rounds + serving-tank cards + the new manual client picker (Piece A) + salle-de-controle objectifs settings + Round-6 fiche + Round-7 pace-check (once budgets entered). Structurally + rollback verified only; needs a manager click-through (subagent can't auth as manager).

---

## §SERVING-TANK OCCUPANCY IN TankSimulator (Round-8) — PM ruling 2026-06-19 (BUILT — but PRODUCER-SIDE, not in TankSimulator; see §ROUND-8 AS-BUILT above for the ratified deviation). Removes the "all serving tanks empty at week start" falsehood (the v1 limitation flagged rounds 3-7; TODO ~L648 in app/tank-simulator.php).
