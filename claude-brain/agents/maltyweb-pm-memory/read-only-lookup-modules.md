...
- 🔴 **RISK:** a whole-tree `bin/deploy.sh` by the parallel session can clobber these shared files AGAIN. RECOMMEND: commit `lookup-panel.js` + the consulter assets when the tree is clean, or coordinate deploys. (This is the deploy-pushes-the-working-tree hazard concretized — multiple sessions, one clone, shared JS.) EQUIP ui+coder+sql+webapp-testing.
...
## 🟢🎨 BOTH CONSULTER SCHEMATICS — SHIPPED + LIVE + SMOKE-PASSED 2026-06-22/23 (surgical per-file rsync, NOT committed). Supersedes the "NOT built" + "Nomenclature en cours de calcul"/"deployed WITHOUT packaging-lookup.php" notes below.
...
- The **10 changed/new files are deployed-LIVE but NOT git-committed yet** (pending Kouros's decision on whether to commit only-his-files on a branch). Same shared-tree-too-dirty-to-separate situation as the UX-fix slice.
