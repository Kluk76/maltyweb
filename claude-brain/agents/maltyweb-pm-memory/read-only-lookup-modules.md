...
## CO₂ READS LOOKUP + v1↔v2 READINGS JOIN-GAP (✅ SHIPPED+LIVE+COMMITTED+PUSHED 2026-06-29, `81cc00b` origin/main, tree 0/0 at commit — PM-VERIFIED)
...
**AS-BUILT (all 3 in commit `81cc00b`, 4 files: packaging-lookup.php +122 / packaging-consulter.css +60 / packaging-consulter.js +125 / scripts/backfill-packaging-readings-v2-key.php +398; lookup-panel.js NOT touched = scope guardrail HELD; SELECT-only API, NO migration; surgical per-file install -m644 -o ubuntu, NOT bin/deploy.sh — 27 dirty files from parallel sessions; md5 3/3; var-not-const honored):**
- **B1b backfill APPLIED** — `scripts/backfill-packaging-readings-v2-key.php --apply` (www-data). **44 v1 parents → 203 readings re-keyed; 100 refused** (96 G1 empty/SKU-code beer, 4 G2 ambiguous Febris-5 + WestCoast-12, 0 G3). All 44 matched = contract/collab (clean ref_recipes.name); every Neb beer refused (v1 entered by SKU code). Snapshot `data/snapshots/bd_packaging_readings-v2-backfill-20260629_074854.json` (203 rows); audit = 203 `update` rows target_table=bd_packaging_readings (PM-VERIFIED live: 203). Idempotent (WHERE packaging_v2_id IS NULL). PM-VERIFIED live: still-NULL v1-keyed dropped **740→537**; Jasper-28 event 2200 now **1** v2-keyed read.
- **B1a BBT fallback LIVE** — packaging-lookup.php returns `bbt_carbonation {co2,o2,racking_event_date}` from `bd_racking_v2` (keyed COALESCE(neb_recipe_id_fk,contract_recipe_id_fk)+batch, racking_destination_type='BBT'), ONLY when run has 0 in-package reads; renderQA shows a SEPARATE "Carbonatation en BBT (soutirage)" block — never conflated with in-package O₂.
- **B2 recent browse LIVE** — `mode=recent&sku_id=N` → last 10 runs (date,batch,format_human server-mapped,units,#reads,avg co2/o2); consulter renders `.day-card` rows w/ data-sku-id+data-batch → drill via EXISTING `activateDayCard` (lookup-panel.js L436 already reads data-sku-id). Smoke (endpoint-logic vs live DB; app Tailscale-only so NO browser run): Jasper-28 sku312 batch28 → co2o2[2200] n=1 co2=4.45 o2=102, BBT suppressed; mode=recent sku312 → 10 rows, counts lot28→1/25→3/24→6/23→12/22→2, lots19-21→0 reads (will show BBT fallback).
- **Precedence:** v2-keyed per-reading rows (native+backfilled) → BBT fallback. (v1-aggregate tier dropped — 2 rows.) REJECTED & not built: per-request v1 string-join in API.

🟡 **OPEN/DEFERRED:** 100 refused v1 parents = **537 still-NULL v1-keyed readings** remain the deferred validation sub-project (migs 243/244 territory) — empty-beer + SKU-code-beer resolution needs operator triage, NEVER force. 🔴 **NO browser smoke (Tailscale-only) → recommend operator eyeball the live Consulter on Jasper lot 28: confirm QA panel renders the 4.45 in-package read + a lot-19/20/21 run shows the BBT fallback block + recent-runs list/drill-down works.**
