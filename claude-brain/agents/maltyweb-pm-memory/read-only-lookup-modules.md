...
## 🟢 CONSULTER-TAB UX FIX — SHIPPED + LIVE + SMOKE-VERIFIED 2026-06-22 (start_open flag + clickable day-cards; surgical 4-file rsync, NOT committed)
Small UX slice on the wort.php "Consulter" tab + the shared lookup panel. Read-only consultation surface — NO SoT/derivation impact (no DB writes, no fiscal lane). Deployed SURGICALLY per-file (NOT bin/deploy.sh) because the maltyweb tree is currently dirty with parallel in-flight work (expéditions/financier/mon-tableau edits, a **timezone migration 438**, and an uncommitted brewing-lookup.php/packaging-lookup.php BOM expansion that is NOT this work). Left in the working tree UNCOMMITTED (tree too entangled to cleanly separate).
...
## 🔴 OPEN — PACKAGING-SIDE PARITY STAGED IN WORKING TREE, NOT DEPLOYED (held 2026-06-22, do NOT clobber)
The identical fix is code-complete LOCALLY for the packaging Consulter surface but **was NOT deployed** — held until the `packaging-lookup.php` BOM expansion (the PACKAGING-LINE-SCHEMATIC slice below) is ready to ship a clean packaging bundle. STAGED, awaiting that owner's deploy:
- `public/modules/packaging.php` (got `start_open` added locally), `public/js/packaging-consulter.js` (identical `data-sku-id`+`data-batch` data-attribute + delegation treatment), `public/css/packaging-consulter.css`.
- 🔴 **FLAG: these 3 files sit in the dirty working tree.** Deploy them WITH the packaging-lookup.php BOM expansion as one clean packaging bundle. Verify they're not accidentally clobbered/reverted by a parallel session before that deploy. EQUIP ui+coder+sql+webapp-testing.

## 🎨 PACKAGING-LINE-SCHEMATIC enrichment (PM-briefed 2026-06-22, NOT built) — the bottling/canning-line analogue of the brewhouse "Schéma de Brassage"
