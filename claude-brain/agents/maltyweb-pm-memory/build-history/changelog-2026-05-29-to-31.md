...
- `is_packaging_line = 1 IFF EXISTS(bd_packaging_v2 WHERE sku_id_fk=s.id) OR format_id=6` (X-cage cardinal invariant per packaging-bom-model.md §`-X` CAGE — X cages are packaging-line outputs by structural definition, regardless of v2 evidence).
...
**Downstream contract recorded (RESUME POINT + derivation-tree-and-schema.md + packaging-bom-model.md cross-links).** Future re-runs of the vessel-commissioning "idle commissioned formats" audit filter by `is_packaging_line=1`. `derive-eshop-consumption.ts` keys off `is_direct_sales=1 AND is_packaging_line=0` (the unboxing tier). `scripts/python/ingest_bd_packaging_v2.py` could add a `is_packaging_line=0` → doc_review_queue safety check (defense against future mis-routing à la BLO/BLA item #2 incident — recorded in scrapping-backlog as a future enhancement, not a blocker).
