...
**SEND ARCHITECTURE (the big new surface — was parked, now wanted):**
- **Scope gate (the ONE operator action):** current DWD grant is `gmail.readonly` ONLY (verified `ingest_email_comm.py:120` + order connector). Sending needs **`https://www.googleapis.com/auth/gmail.send`** added to the SAME domain-wide-delegation client in Google Admin (Security → API controls → Domain-wide delegation → edit the client ID for SA `maltytask-gmail-reader@kouros-infrastructure...` → append `gmail.send` to the comma-sep scope list; same client, same SA, no new SA). `gmail.send` is narrower than `gmail.compose`/`gmail.modify` and sufficient for `users.messages.send` (it does NOT grant read — we already have readonly, so the pair = read+send). **Verify-probe from VPS** (mirror the readonly probe): impersonate kouros@ with scope `gmail.send`, attempt a `users.messages.send` of a trivial draft to a throwaway/self address (or just instantiate the service with the send scope and call `users.getProfile` — a 403 `unauthorized_client`/"not authorized for any of the scopes" = grant NOT yet live; success = granted). Kouros must do the Admin step himself if the probe 403s.
- **Send path:** Gmail API `users.messages.send` impersonating the linked box (v1 = kouros@). A NEW connector/handler (PHP cannot call the Python Gmail client directly — either a small `scripts/python/send_email_comm.py` invoked by the PHP API via shell-exec with absolute python path per the empty-PATH rule, OR a thin send shim; PM lean = a Python sender sibling to ingest_email_comm.py, called by sf-comm-thread.php's new `send_reply` action). **Confirm-then-send: write the comm_messages row ONLY on Gmail 2xx success**; failures surface to the operator UI ([[feedback_ingest_failures_must_surface]] applies to SEND too — no silent log).
- **Reply threading (correct in BOTH Gmail and comm_threads):** set the outbound MIME `In-Reply-To:` and `References:` headers from the original message's `Message-ID` (chain the References), pass Gmail `threadId` = the thread's `gmail_thread_id` so Gmail nests it natively; on send, capture the returned Gmail message id + the generated Message-ID and INSERT a comm_messages row direction='out' into the SAME comm_threads row.
- **SCHEMA DELTA = mig 414+ (re-`--status`; head ≥414, next-free is whatever --status shows):** add `'sent'` to comm_messages.source ENUM (currently `('gmail','manual')` → `('gmail','manual','sent')`) so a maltyweb-originated reply is distinguishable from a captured outbound (gmail) and a typed note (manual); add a `send_status ENUM('draft','sent','failed') NULL` + `send_error VARCHAR(512) NULL` so a failed send is visible/retryable in the UI. Minimal — no new table. (`direction='out'` already exists.)
- **Send-as identity:** v1 sends as kouros@ — acceptable for proof, but FLAG to Kouros: replies will appear to come from a personal box. Consider From/Reply-To = a role address (`achats@`/`contact@`) so the supplier-facing identity is institutional, not personal. Decision for Kouros.
- **Direct-send vs draft-for-review:** PM lean = **confirm-then-send** (operator types reply, sees a confirm, send fires, row written on success). Draft-for-review is heavier (a draft state + a second approve action) — defer unless Kouros wants a manager-approval gate.

**DRAG-DROP DOCUMENT ATTACH:**
- **(a) Docs we generated FOR the supplier — FINDING: there is NO outbound-doc generation in the app today.** `supplier_cert_documents` are docs received FROM suppliers (certs/COAs/spec sheets) LINKED to an existing doc_files row, not docs we produce. So "attach a doc we made for them" is currently ASPIRATIONAL — there is no such corpus. What CAN be offered today: docs already ASSOCIATED with the supplier = (i) supplier_cert_documents.doc_file_id_fk, (ii) any doc_files referenced by comm_message_docs on that supplier's threads, (iii) invoices/DNs tied to the supplier via inv_deliveries→doc_files. Provide a "Documents du fournisseur" picker that UNIONs those sources by supplier_id. (If Kouros wants generated quotes/specs, that's a separate outbound-doc-generation arc.)
...
- **P2.5 — tabbed-fiche layout refactor FIRST (pure UI, NO new scope, ships independently).** Convert openFiche to TabStrip→Panel, fold Fiche/Évaluation/Discussions, move the existing `.sf-disc-*` timeline into the Discussions tab's two-pane shell (still read+note only). This is safe to ship WHILE the gmail.send grant is being processed — it adds zero send capability. Ship this independently.
- **P3-send — outbound reply + attach (GATED on the gmail.send grant + the verify-probe passing).** mig (source ENUM + send_status); Python sender (multipart, In-Reply-To/References/threadId); sf-comm-thread.php `send_reply` action (confirm-then-send, row-on-success, failure→UI); drag-drop zone + supplier-doc picker. EQUIP sql+coder+ui+webapp-testing (+ a light parser-coder envelope for MIME assembly).
- Customer side (P3-cust, the original P3) still the bigger lift, still deferred.

**OPERATOR DECISIONS to put to Kouros (this expansion):**
1. **Send-as address** — kouros@ personal, or a role address (achats@/contact@) so suppliers see an institutional From? (PM recommends role address.)
2. **Direct-send vs draft-for-review** — confirm-then-send (PM lean) or a manager-approval draft state?
3. **Drag-drop doc sources** — confirm the union (certs + thread docs + invoices/DNs) is the right "supplier documents" corpus; and CONFIRM there is no existing "docs we generate FOR suppliers" he expected (there isn't — flag the gap).
4. **Who can send** — admin-only, or managers too? (Capture is currently admin-triage + manager-read; sending is a higher-trust action — recommend admin-only v1, widen later.)
5. **Which mailbox sends** — if multi-mailbox capture ever lands, which box is the send identity? (v1 kouros@.)
