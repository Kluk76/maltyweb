...
## ✅ RESET-LINK BOUNCE BUG — FIXED + DEPLOYED-LIVE 2026-06-29 (code-only, NO mig; 🔴 DEPLOYED-NOT-COMMITTED)

**Symptom (Kouros, on his PHONE):** forgot password, clicked the admin-reset email link, but it "simply connects me, nothing happens" — never showed the set-password form. **Root cause (PM-briefed, code-confirmed):** `public/set-password.php` had an unconditional guard at the very top — `if (current_user() !== null) { header → /modules/mon-tableau.php; exit; }` — that ran **before any token lookup**. Kouros's 90-day `mt_remember` cookie silently auto-authenticated him → `current_user()` non-null → 302-bounce to dashboard before the token was ever read or the form rendered. Token NOT consumed (only bounced), so the SAME link keeps bouncing on every click. Guard is correct for INVITE (new user never logged in) but structurally broke RESET (existing, possibly-logged-in user) — `purpose='reset'` was built but never exercised against a live session (mig 262 even labelled it "future use").

**Fix (Sonnet coder, php-security mindset) — guard now at `public/set-password.php` lines 46–62:** when `current_user() !== null`, **pre-look-up the token first**: null/invalid token → keep the old dashboard bounce; **valid token → `auth_logout()` (revokes THIS device's remember token + destroys session, `app/auth.php:255`) then `maltytask_session_start()`, fall through to the form.** Covers self-reset AND the cross-user edge identically (Kouros's ruling = "honour the token, but tear down the current session first" — no distinct "logged in as someone else" message; generic `$invalidHtml` stays the only failure surface). POST handler UNTOUCHED. All invariants verified intact: CSRF, rate-limit, Argon2id, TOCTOU consume gate, `rt_revoke_all` on reset, freshUser SELECT carries manager_scope/access_preset_id_fk/tour_seen_at, no-enumeration.

**Deploy:** surgical per-file rsync ONLY (maltyweb tree dirty with a parallel session's topbar/financier.js/reglages-generaux.php work → NO whole-tree `bin/deploy.sh`); `php8.1 -l` clean on VPS; php8.1-fpm reloaded; LIVE in prod. **No migration** (table/ENUM/service pre-existed). 🔴 **NOT COMMITTED** — commit `public/set-password.php` by **pathspec** when Kouros asks (parallel uncommitted work in the tree; whole-tree deploy WOULD revert it). **NOT smoke-tested with any account** (standing real-hash incident); verification plan = Kouros's original reset token was never consumed, so within its 72h TTL the SAME emailed link should now work (real user, real flow, no synthetic accounts → no fail2ban risk).

🔴 **OPEN FOLLOW-UP (flagged to Kouros, candidate build for this arc):** NO self-service "mot de passe oublié" link on `login.php` — a fully-logged-out non-admin who forgets their password is STUCK until an admin reissues. The fix above only helps a user STILL authenticated (live session/remember cookie); the logged-out-and-forgotten case has no path. Build candidate = public "forgot password" form → `invite_create(purpose='reset')` → email link (reuses the now-working set-password.php reset path). Gate carefully (rate-limit, no-enumeration; Tailscale-only already constrains exposure).
