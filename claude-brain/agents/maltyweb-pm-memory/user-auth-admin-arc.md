...
**`user_can_access($page_key,$u)` in `app/auth.php` — resolution ORDER (load-bearing; REORDERED 2026-06-09, commit `aefbaad`):**
  ...
  3. **explicit `user_page_access` override (granted 0/1) for this (user,page)** — grant=1 OVERRIDES the floor (returns true even if user-rank < min_role); grant=0 ALWAYS denies. [now BEFORE the floor]
  4. **ROLE FLOOR: `_role_rank(user) < _role_rank(page.min_role)` → false.** ranks viewer0/operator1/manager2/admin3.
  5. preset membership (`ref_access_preset_pages` via `users.access_preset_id_fk`). [unchanged]
...
- **Thierry = user id=22 now:** `role=manager`, **`manager_scope=NULL`** (pure read-only per Kouros ruling 1), `access_preset_id_fk=10`.
- **Code shipped:**
...
- **🔴 CROSS-ARC DEPENDENCY STILL OPEN (flag to the financier-arc owner):** when the Dynamic-Financier seal/restate WRITE ships, gate it so the CFO passes. The parallel `financier.php` rework already added **`$canSealFiche = is_admin() || manager_can('finance')`** — but `manager_can('finance')` requires a **'finance' scope/domain that may not exist yet**; the CFO (scope=NULL) will NOT pass it. **The CFO needs an explicit finance-write capability (preset-membership-based or a real 'finance' scope wired into `manager_can`), or the seal button stays hidden for him.** Carried in the financier arc index banner as the seal-gate cross-dependency.
...
- **LIVE-PROVEN 2026-06-09:** Olivier renamed to `OlivierBarral` (id 19); his subsequent FG-stocktake edit audit-logged the actor as `OlivierBarral` — rename propagated correctly through to the audit trail. See fulfilment-expeditions-arc.md §2026-06-09 GO-LIVE.
...
5. **`ref_kpi_trackers` id 266 "Jours de couverture stock (MP + PF)" actually returned RM stock CHF**
   ...
   (gap #58 now fixed). → kpi-tracker-catalog.md.
...
## 🗓 2026-06-07 EVENING — SESSION-PAYLOAD DEFECT #3 + SUB-PAGE ACL HARDENING (Mon tableau v2 batch; → kpi-tracker-catalog.md §2026-06-07 EVENING for the KPI side)
...
- **✅ P3 KPI recap emails — SHIPPED + CRON ACTIVATED** (mig 277 `user_kpi_recap_subs` + `send-kpi-recap.php`; cron flipped LIVE 2026-06-07 `1d8672b` after operator confirmed inbox rendering) — **the user-account arc's LAST build is DONE**; everything below is operator workflow or fast-follow. Detail → kpi-tracker-catalog.md.
...
- **Onboarding rollout:** SELF-TEST PASSED 2026-06-07 (TestOnboard, §above — 5 defects fixed). PROGRESS: control-room PCs + managers Gonzalo + Yves operational via Tailscale (2026-06-08); **operator Olivier (id 19, `OlivierBarral`, preset=marketing id 8) onboarded + logged in + ran a live FG stocktake 2026-06-09** (on his own personal Windows machine — see §2FA driver above + fulfilment-expeditions-arc.md §2026-06-09 GO-LIVE). Remaining = remaining real invites; SKIP Joelson until joelson@lanebuleuse.ch mailbox exists. First email each user gets is the WELCOME one (1c).
